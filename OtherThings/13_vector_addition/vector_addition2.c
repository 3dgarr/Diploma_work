...
int main (int argc, char *argv[]) {
    ...
    long elements_to_compute = (rank == processors - 1) ? N - n_per_proc * (processors - 1) : n_per_proc;

    for(i = 0; i < elements_to_compute; i++)
        cp[i] = ap[i] + bp[i];

    MPI_Gather(cp, elements_to_compute, MPI_INT, c, elements_to_compute, MPI_INT, 0, MPI_COMM_WORLD);
